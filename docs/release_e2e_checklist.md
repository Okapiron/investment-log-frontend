...
## 5.1 ケースE: 楽天CSV取込（公開v1の主導線）
- [ ] `Settings > 証券会社 CSV取込` で既定が「楽天証券」になっている
- [ ] `SBI証券` が公開v1で正式対応に見えない（非表示またはβ表示）
- [ ] 楽天CSVをアップロードして `プレビュー` が表示される
- [ ] 取込前に `作成予定 / スキップ / エラー` 件数が表示される
- [ ] エラー行に「何が不足/不正か」が日本語で表示される
- [ ] `この内容で取り込む` 実行後に `作成 / 更新 / スキップ / エラー` 件数が表示される
- [ ] 同じCSVを再取込しても新規重複作成されず、更新扱いになる
- [ ] 取込完了後に `/analysis` へ遷移できる
- [ ] `/analysis` で「今の売買スタイル」「次の改善点」「勝ち負け傾向」が表示される
- [ ] 手入力Tradeを残したままCSV取込Tradeが共存できる
